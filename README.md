...
| `employees` / `employee_disaster_badges` | `scripts/seed-employees.ts` | 인사 변동 시 스크립트로 갱신 |
...
```bash
npm run seed-employees   # employees + employee_disaster_badges 시드 (인사 변동 시 재실행, upsert라 중복 실행 무관)
