...
- **훈련 참여인원설정에서 사람별 "TTS 전화 받기" 지정 (2026-07-23 추가)**: 지휘본부 "훈련 참여인원 설정"/"2차 소집 대원" 패널에서 참여 체크박스와 별개로 각 사람마다 TTS 체크박스를 켤 수 있습니다. 한 명이라도 켜면 그 훈련 발령/승격은 배지(통제) 기준 대신 **그 목록만** TTS 대상이 됩니다. 목록 툴바의 "📞 파트장 전체 TTS" 버튼으로 전체 파트장을 한 번에 켤 수 있습니다. 아무도 안 켜면(기존과 동일하게) 배지 기준으로 동작합니다.
